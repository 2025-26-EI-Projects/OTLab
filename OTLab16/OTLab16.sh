...
zeek_host_dir="./scripts/solutions"

attack_scenarios=(scan fingerprint spoof)


ot_subnet="192.168.20.0/24"
outstation_ip="192.168.20.10"
master_ip="192.168.21.20"
attacker_ip="192.168.21.30"
...
check_script_layout() {
    ...
    if [ "$missing" -eq 1 ]; then
        printf "\033[31m✘ Lab tree incomplete. Run this script from the OTLab16/ directory.\033[0m\n"
        exit 1
    fi
}
  ...
  $attacker_container_name:
    ...
    command: >
      bash -c '
        ...
        ip route replace default via 192.168.21.100 &&
        sysctl -w net.ipv4.ip_forward=1 &&
        echo "[ATTACKER] Ready. Awaiting -attack / -incident triggers from host." &&
        tail -f /dev/null'
...
# ----------------------------------------
# -incident: replay the full kill-chain so the student gets a populated notice.log
# to open the incident from. Same scenarios as OTLab15, fired back-to-back.
run_incident() {
    if ! container_exists "$attacker_container_name"; then
        printf "\033[31m[Error]\033[0m Attacker container not running. Run -start first.\n"
        exit 1
    fi
    printf "\033[1;33m[Working]\033[0m Replaying the full kill-chain to open the incident. . .\n"
    ...
    for scenario in "${attack_scenarios[@]}"; do
        run_attack "$scenario"
        printf "\033[34m[Information]\033[0m Scenario '%s' done. Pausing before the next stage. . .\n" "$scenario"
        sleep 5
    done
    printf "\033[32m✔ Kill-chain replayed (scan → fingerprint → spoof). The incident is open.\033[0m\n"
...
apply_containment_rules() {
    if ! container_exists "$ews_container_name"; then
        printf "\033[31m[Error]\033[0m EWS container %s not running — run -start first.\n" "$ews_container_name"
        exit 1
    fi

    # Idempotent: remove any prior copy of this rule first, then insert it at the top.
    docker exec "$ews_container_name" iptables -D FORWARD -s "$attacker_ip" -d "$ot_subnet" -j DROP 2>/dev/null
    docker exec "$ews_container_name" iptables -I FORWARD 1 -s "$attacker_ip" -d "$ot_subnet" -j DROP
}

revert_containment_rules() {
    if ! container_exists "$ews_container_name"; then
        return 0
    fi
    # Remove every copy of the containment rule (idempotent).
    while docker exec "$ews_container_name" iptables -C FORWARD -s "$attacker_ip" -d "$ot_subnet" -j DROP 2>/dev/null; do
        docker exec "$ews_container_name" iptables -D FORWARD -s "$attacker_ip" -d "$ot_subnet" -j DROP 2>/dev/null
    done
}
...
case "$1" in
    ...
    -contain)
        ...
        check_requirements
        detect_environment
        printf "\033[1;33m[Working]\033[0m Applying reference containment (isolate, do not shut down). . .\n"
        printf "\033[34m[Information]\033[0m Dropping attacker %s -> OT %s; preserving master %s -> outstation %s.\n" \
            "$attacker_ip" "$ot_subnet" "$master_ip" "$outstation_ip"
        apply_containment_rules
        printf "\033[32m✔ Containment applied. Verify with Zeek: attacker flows stop, polling continues.\033[0m\n"
    ...
    -restore)
        ...
        check_requirements
        detect_environment
        printf "\033[1;33m[Working]\033[0m Lifting containment — returning to the clean monitored state. . .\n"
        revert_containment_rules
        printf "\033[32m✔ Containment lifted. Re-run your Zeek baseline check to sign off recovery.\033[0m\n"
        ;;
    *)
        show_banner
        echo "Usage: $0 -start [kali|ubuntu] | -stop | -clean | -run | -restart | -status | -attack <scenario> | -incident | -contain | -restore"
        echo ""
        ...
        echo "  -clean     Remove containers, volumes, and network (reverts all iptables rules)"
        echo "  -stop      Stop all containers"
        echo "  -restart   Restart previously stopped containers"
        echo "  -status    Show current containers status"
        echo "  -attack    Fire a single controlled attack scenario from $attacker_container_name"
        echo "             Valid scenarios: ${attack_scenarios[*]}"
        echo "  -incident  Replay the full kill-chain (${attack_scenarios[*]}) to open the incident"
        echo "  -contain   Apply reference containment: surgical DROP of the attacker into OT,"
        echo "             preserving the legitimate master->outstation poll"
        echo "  -restore   Lift containment and return to the clean monitored state"
        exit 1
        ;;
esac
